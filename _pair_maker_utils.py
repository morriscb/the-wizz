...
class TargetPairFinder(RawPairFinder):
    ...
    def find_pairs(self, max_scale):
        """Main functionality of the TargetPairFinder class. Given the input
        data, we compute the number of target objects within a fixed angular
        scale around themselves (aka an auto-correlation).
        ------------------------------------------------------------------------
        Args:
            max_scale: float value of the maximum scale to run the pair finder.
                Units are physical kpc
    ...
    def write_to_hdf5(self, hdf5_file, group_name):
        """Method to write the over-density and raw number counts. These
        "pair files" are the heart of The-wiZZ and allow for quick computation
        and recomputationof clustering redshift recovery PDFs.
        ------------------------------------------------------------------------
        Args:
            hdf5_file: Open HDF5 file object from h5py
            scale_name: Name of the specific scale that was run. This will end
